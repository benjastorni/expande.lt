expande.lt = function(puente, muestra, estratos, cc, sp, mas = F, filt.est = NULL, g
raph = F){
...
ch(is.na(puente$AREA_BARR))), " lance(s) sin dato de area barrida"))
}
...
cols_vary = "slowest", # De esta forma especie y kilogramo quedan v
inculados
names_to = ".value", # Nombre de las columnas de salida. Ignorar
names_pattern = "(.)", # Separación de nombres. Ignorar
values_drop_na = T) %>% # Elimina todos los NA estructurales durant
e el pivot
...
mutate(KG = ifelse(is.na(KG), 0, KG), # Las capturas que tuvieran NA las paso a
cero para considerarlas
...
stop("La base MUESTRA no tiene el formato adecuado!\n Hay mas muestras que lanc
es!")
}
# Uno las bases de puente_capt que tenia con las primeras columnas de muestra y lo
s estratos.
pue_capt_mues_est <- puente_capt %>%
left_join(muestra_filtro[, 1:11], by = c("LANCE", "ESPECIE")) %>%
left_join(estratos, by = "ESTRATO") %>%
mutate(PESO_MUES = ifelse(is.na(PESO_MUES) | TALLA_1 %in% c(0, NA), 0, PESO_MUES
)) # Filtro aquellas muestras sin informacion real de tallas o con talla ausente
...
suma_muestra_kg = sum(PESO_MUES), # Sumo los pesos de muestra en kil
os
suma_captura_kg = sum(KG), # Sumo las capturas en kilos
capc_mues_kg = sum(KG[PESO_MUES > 0]), # Sumo las capturas con muest
ra en kilos
nlance = length(LANCE), # Veo cantidad de lances en cada estrato
lances_cap = length(LANCE[KG > 0]), # Veo la cantidad de lances con
captura
mdens_ton.mn2 = round(mean(DENSIDAD), digits = 5), # Calculo la dens
idad media en toneladas por milla nautica cuadrada
var_dens = var(DENSIDAD), # Varianza de la densidad
var_mdens = var_dens / nlance, # Varianza de la densidad media
area_mn2 = sum(unique(AREA))) # Area total en millas nauticas cuadra
das
biom_total <- data.frame(biom_total_ton = pm_sum$mdens_ton.mn2 * pm_sum$area_mn2
, # Biomasa total
var_biom_tot = pm_sum$var_mdens * (pm_sum$area_mn2 ^ 2)
, # Varianza de la biomasa
desvio_biom_tot = sqrt(pm_sum$var_mdens * (pm_sum$area_
mn2 ^ 2))) # Desvio de la biomasa
} else{
...
lances_cap = length(LANCE[KG > 0]), # Veo la cantidad de lances con
captura
n_cuadriculas = unique(CUADRICULS), # Numero de cuadriculas para cad
a estrato
mdens_ton.mn2 = round(mean(DENSIDAD), digits = 5), # Calculo la dens
idad media en toneladas por milla nautica cuadrada
var_mdens = round((var(DENSIDAD) / nlance) * (1-(nlance / n_cuadricu
las)), digits = 5), # Varianza de la densidad media
area_mn2 = unique(AREA)) %>% # Area de cada estrato en millas nautic
as cuadradas
mutate(biom_est_ton = (n_cuadriculas / sum(n_cuadriculas)) * mdens_ton.mn2 * s
um(area_mn2), # Calculo la biomasa de cada estrato en toneladas
var_biom_est = ((n_cuadriculas / sum(n_cuadriculas)) ^ 2) * (sum(area_m
n2) ^ 2) * var_mdens) # Varianza de la biomasa en cada estrato
# Calculo la biomasa total y su varianza
biom_total <- data.frame(biom_total_ton = sum(pme_sum$biom_est_ton), # Biomasa t
otal
var_biom_tot = sum(pme_sum$var_biom_est, na.rm = T), #
Varianza de la biomasa
desvio_biom_tot = sqrt(sum(pme_sum$var_biom_est, na.rm
= T))) # Desvio de la biomasa
...
factor_lance = KG / PESO_MUES, # Calculo factores de expansion de los
lances
factor_biomasa = biom_total_ton / (capc_mues_kg / 1000),
coef_exp = factor_lance * factor_biomasa) %>% # Calculo el coeficiente
de expansion
select("COD_CAMP", "LANCE", "KG", "PESO_MUES", "capc_mues_kg", "biom_total_ton
", "factor_lance", "factor_biomasa", "coef_exp")
} else{
ponds = pue_capt_mues_est %>%
...
factor_lance = KG / PESO_MUES, # Calculo factores de expansion de los
lances
factor_estrato = biom_est_ton / (capc_mues_kg / 1000), # Calculo factor
es de expansion de los estratos
factor_total = biom_total_ton / biom_estc_mues_ton, # Calculo factores
de expansion totales
coef_exp = factor_lance * factor_estrato * factor_total) %>% # Calculo
el coeficiente de expansion
select("COD_CAMP", "LANCE", "KG", "PESO_MUES", "biom_est_ton", "capc_mues_kg",
"biom_total_ton", "biom_estc_mues_ton", "factor_lance", "factor_estrato", "factor_to
tal", "coef_exp")
}
# Warning para avisar que los pesos de muestra son mayores a los pesos de captura
en un 5%
...
################# TALLAS E INDIVIDUOS #################
################################################################
# Separo las tallas y las paso a formato longer. Misma sintaxis que lo hecho en ca
pturas. Luego, extraigo la informacion en los codigos de talla
...
mutate(lt_cm = as.numeric(str_sub(TALLA_CODIGO, -15, -13)), # Extraigo el largo
total en cm
n_machos = as.numeric(str_sub(TALLA_CODIGO, -12, -10)), # Extraigo el num
ero de machos
n_hembras = as.numeric(str_sub(TALLA_CODIGO, -9, -7)), # Extraigo el nume
ro de hembras
n_indet = as.numeric(str_sub(TALLA_CODIGO, -6, -4)), # Extraigo el numero
de indeterminados
n_total = as.numeric(str_sub(TALLA_CODIGO, -3, -1))) %>% # Extraigo el to
tal de individuos
filter(n_total > 0) # Filtro los datos que no tuvieran informacion real sobre es
a talla
...
####################################################################
# Al numero de individuos por lance y clase de tallas lo multiplico por el coefici
ente de expansion que corresponde
individuos_pond <- tallas %>%
right_join(ponds, by = "LANCE") %>%
select(COD_CAMP, LANCE, lt_cm, n_machos, n_hembras, n_indet, # Selecciono las co
lumnas de interes
n_total, coef_exp) %>%
...
coef_exp), # Pondero
pob_indet = floor(n_indet * coef_exp), pob_total = floor(n_total * coef_e
xp))
# Warning que avisa si hay errores en las tallas
tallas_na <- which(is.na(individuos_pond$lt_cm))
if(length(tallas_na) > 0){
warning(paste0("Lance(s) ", individuos_pond$LANCE[tallas_na], " contiene codigos
de tallas con valores erróneos.\n Revise los codigos de talla"))
...
vise los códigos de tallas en lance(s) ", unique(individuos_pond[totales, "LANCE"]),
" en la base MUESTRA\n Existen ", sum(totales), " códigos de talla que no tienen el
formato adecuado!"))
}
# Agrupo por tallas entre todos los estratos y calculo los totales poblacionales y
muestreados
rango_lt = data.frame(lt_cm = seq(min(individuos_pond$lt_cm), max(individuos_pond$
lt_cm), by = 1))
...
relocate(COD_CAMP) %>%
arrange(lt_cm) %>%
replace_na(list(machos_mues = 0, hembras_mues = 0, indet_mues = 0, total_mues =
0, tot_machos = 0, tot_hembras = 0, tot_indet = 0, tot_total = 0))
...
labs(title = "Area barrida vs Lances", subtitle = "Recta roja en media de Area
barrida") +
theme_classic()
...
grid.arrange(plot_capturas, plot_area_barr, ncol = 2) # Acomodo los de captura y
area barrida en un solo panel
print(plot_densidad)
}
######################################################
...
if(mas == T){
resultados = list(resumen_resultados = as.data.frame(pm_sum), biomasa_total = bi
om_total, distribucion_longitudes = as.data.frame(frecuencia), factores_expansion =
as.data.frame(ponds))
resultados
} else{
resultados = list(resumen_resultados = as.data.frame(pme_sum), biomasa_total = b
iom_total, distribucion_longitudes = as.data.frame(frecuencia), factores_expansion =
as.data.frame(ponds))
resultados
}
}